...
def transform_for_kv_store(data: csv.DictReader) -> list[dict]:
    ...
    for row in data:
        kv_data.append(
            {
                "ip_address": row["Device IP Address"],
                "device_name": row["Device Name"],
                "date": row["Date"],
                "location": row["Location"],
                "power_budget": row["Power Budget"],
                "power_usage": row["Power Usage"],
                "power_usage_percent": row["Power Usage(%)"],
                "used_ports_count": row["Used Ports Count"],
                "fast_poe_enabled": row["Fast PoE Enabled Ports Count"],
                "persistent_poe_enabled": row["Persisent PoE Enabled Ports Count"],
                "policing_poe_enabled": row["Policing PoE Enabled Ports Count"],
                "high_priority_port_count": row["High Priority Ports Count"],
